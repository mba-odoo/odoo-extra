...
class RunbotRepo(models.Model):
    ...
    def update_git(self):
        ...
        git_refs = self.git(['for-each-ref', '--format', fmt, '--sort=-committerdate', 'refs/heads', 'refs/pull'])
    ...
    def scheduler(self):
        ...
        build_to_schedule = Build.search(domain_host + [('state', 'in', ['testing', 'running'])])
        build_to_schedule.schedule()
        ...
        sorted_by_sticky[running_max:].kill()
        sorted_by_sticky.reap()
...
class RunbotBuild(models.Model):
    ...
    result = fields.Char(default="")  # ok, ko, warn, skipped, killed
    pid = fields.Integer()
    ...
    def create(self, values):
        ...
        duplicate_biild = self.search(domain, limit=1)

        if len(duplicate_biild):
            extra_info.update({'state': 'duplicate', 'duplicate_id': duplicate_biild.id})
            duplicate_biild.write({'duplicate_id': build.id})
    ...
    def checkout(self):
        for build in self:
            ...
            modules_to_test = self.filter_modules(modules_to_test,
